...
var fsPromise = _interopRequireWildcard(require("node:fs/promises"));
  ...
  _createClass(Bootcamp, null, [{
    ...
    value: function register(input) {
      ...
      _fs["default"].readFile(path, function (err, data) {
        ...
        _fs["default"].writeFile(path, JSON.stringify(employees), 'utf8', function (err) {
          if (err) {
            console.error(err);
          } else {
    ...
    value: function login(input) {
      ...
      var employees = [];
      fsPromise.readFile(path, 'utf8').then(function (data) {
        try {
          employees = JSON.parse(data);
        } catch (err) {
          console.error(err);
        ...
        var user = employees.find(function (user) {
          return user.name === name && user.password === password;
        });
        if (user) {
          user.isLogin = true;
          return fsPromise.writeFile(path, JSON.stringify(employees), 'utf8').then(function () {
            console.log('Berhasil Login');
          });
        } else {
          // cek dengan some method
          var userExists = employees.some(function (user) {
    ...
    key: "addSiswa",
    value: function () {
      var _addSiswa = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(input) {
        var _input$split5, _input$split6, siswaName, trainerName, employees, admin, trainerIndex;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              _input$split5 = input.split(','), _input$split6 = _slicedToArray(_input$split5, 2), siswaName = _input$split6[0], trainerName = _input$split6[1];
              _context.prev = 1;
              _context.next = 4;
            ...
            case 4:
              ...
              admin = employees.find(function (user) {
                return user.role === 'admin' && user.isLogin;
              });
              if (!admin) {
                _context.next = 20;
                break;
              }
              trainerIndex = employees.findIndex(function (user) {
                return user.name === trainerName && user.role === 'trainer';
              });
              if (!(trainerIndex !== -1)) {
                _context.next = 17;
                break;
              ...
              employees[trainerIndex].students.push({
                name: siswaName
              });
              _context.next = 14;
...
var _default = exports["default"] = Bootcamp;